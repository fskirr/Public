...
#   as an item:     sbr[<ITEM_TO_QUERY>,<JOBID>]cd 
#
# Add to Zabbix Agent
...
switch ($ITEM) {
  "DiscoverTasks" {
$apptasks = Get-BEJob -Status Active
if (!$apptasks) {$apptasks = Get-BEJob -Status Scheduled}
...
$nametask = Get-BEJobHistory -Name "$name1" | Select -last 1
